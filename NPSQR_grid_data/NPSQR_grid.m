clear all
rng(1)
data = xlsread('transformed_data_grid.xls');
...
division_transformed = [0,.2,.5,.75,.8,1]; % quantiles used as grid in given data
%(Note that in the given data here corresponding to each value of x (in the first row), the
% value 1 represents between which 2 quantile grid-points, corresponding y 
% belongs).


positions = zeros(data_size,1);
for i = 1:data_size
    positions(i) = find(data(i,2:size(data,2)));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Warm-start %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%  GCDVSMS parameters %%%%%
...
for ii = 1:max_runs
    ...
    for i=1:no_iter
        ...
        current_lh_1 = fun(c,Basis,theta,x_array,positions,division_transformed);
        ...
        for j = 1:n
            ...
            for positive_change_loc = 1:M
                ...
                if(min(ge(possibility_pos,0)) == 0 || isequal(possibility_pos,theta(:,j)) == 1)
                    ...
                    total_lh_pos(positive_change_loc) = fun(c,Basis,proxy_coef_theta_pos,x_array,positions,division_transformed);
                end
            ...
            for negative_change_loc = 1:M
                ...
                if(min(ge(possibility_neg,0)) == 0 || isequal(possibility_neg,theta(:,j)) == 1)
                    ...
                    total_lh_neg(negative_change_loc) = fun(c,Basis,proxy_coef_theta_neg,x_array,positions,division_transformed);
                end
...
start_log_lh = -fun(c,Basis,theta,x_array,positions,division_transformed);


all_coef_diff = cell(1,mcmc_no);
total_lh_old = -fun(c,Basis,theta,x_array,positions,division_transformed);
acceptance_no = 1;
...
for i=2:mcmc_no
    ...
    for j = 1:(p+2)
        ...
        total_lh = -fun(c,Basis,coef_diff,x_array,positions,division_transformed);
        if(isreal(total_lh) == 0)
            break;
        end
...
q_values = zeros(19,101);
for j = 1:19
    for k = 1:101
        q_values(j,k) = q_tau_x_axis(c,Basis,theta_mats,tau(j),(k-1)*0.01);
    end
end
...
for i = 1:19
    tau(i) = i*.05;
    h(i)=plot(linspace(0,1,101),q_values(i,:),'color',col(i,:));
end
axis([0 1 0 1])
...
title(['Simultaneous quantiles (NPSQR Grid Data)'])
xlabel('X')
ylabel('Y')
hold off
