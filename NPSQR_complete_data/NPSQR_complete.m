...
for ii = 1:max_runs
    ...
    for i=1:no_iter
        ...
        current_lh_1 = -log_like(Basis,linear_basis, theta,x_array,y_array);
        ...
        for j = 1:n
            ...
            for positive_change_loc = 1:M
                ...
                if(min(ge(possibility_pos,0)) == 0 || isequal(possibility_pos,theta(:,j)) == 1)
                    ...
                    total_lh_pos(positive_change_loc) = -log_like(Basis,linear_basis, proxy_coef_theta_pos,x_array,y_array);
                end
            ...
            for negative_change_loc = 1:M
                ...
                if(min(ge(possibility_neg,0)) == 0 || isequal(possibility_neg,theta(:,j)) == 1)
                    ...
                    total_lh_neg(negative_change_loc) = -log_like(Basis,linear_basis, proxy_coef_theta_neg,x_array,y_array);
                end
...
start_log_lh = log_like(Basis,linear_basis, theta,x_array,y_array);
coef_diff_old = theta; 
coef_diff = theta;
...
total_lh_old = log_like(Basis,linear_basis, theta,x_array,y_array);
acceptance_no = 1;
...
title(['Simultaneous quantiles (NPSQR Complete Data)'])
xlabel('X')
ylabel('Y')
hold off
