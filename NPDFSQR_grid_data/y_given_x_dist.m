function answer = y_given_x_dist(Basis,theta, x_value, y_value)
...
p = size(theta,2) - 2; % x basis coeffs
    ...
    sum_tot = 0;
    for j = 1:(p+2)
        cumsum_here = transpose([0;cumsum(theta(:,j))]);
        value(j) = cumsum_here*Basis_y_now(:,1)*y_value^2 + ...
            cumsum_here*Basis_y_now(:,2)*y_value + ...
            cumsum_here*Basis_y_now(:,3);
