...
for ii = 1:max_runs
    ...
    for i=1:no_iter
        ...
        current_lh_1 = -likelihood_dist(Basis, theta,x_array,positions,division_transformed);
        ...
        for j = 1:n
            ...
            for positive_change_loc = 1:M
                ...
                if(min(ge(possibility_pos,0)) == 0 || isequal(possibility_pos,theta(:,j)) == 1)
                    ...
                    total_lh_pos(positive_change_loc) = -likelihood_dist(Basis, proxy_coef_theta_pos,x_array,positions,division_transformed);
                end
            ...
            for negative_change_loc = 1:M
                ...
                if(min(ge(possibility_neg,0)) == 0 || isequal(possibility_neg,theta(:,j)) == 1)
                    ...
                    total_lh_neg(negative_change_loc) = -likelihood_dist(Basis, proxy_coef_theta_neg,x_array,positions,division_transformed);
                end
...
total_lh_old = likelihood_dist(Basis, theta,x_array,positions,division_transformed);
acceptance_no = 1;
...
for i=2:mcmc_no
    ...
    for j = 1:(p+2)
        ...
        total_lh = likelihood_dist(Basis, coef_diff,x_array,positions,division_transformed);
        %total_lh = log_like(Basis,linear_basis, coef_diff,x_array,y_array);
...
title(['Simultaneous quantiles (NPDFSQR Grid Data)'])
xlabel('X')
ylabel('Y')
hold off
